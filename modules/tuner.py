...
def model_builder(hp: kt.HyperParameters) -> keras.Model:
    ...
    for feat in NUMERICAL_FEATURES:
        inputs[transformed_name(feat)] = layers.Input(
    ...
    for feat in CATEGORICAL_FEATURES:
        ...
        embed = layers.Embedding(input_dim=150, output_dim=8)(cleaned)
        cat_embeddings.append(layers.Flatten()(embed))

    num_layers = [inputs[transformed_name(f)] for f in NUMERICAL_FEATURES]
    all_features = layers.concatenate(num_layers + cat_embeddings)

    units_1 = hp.Int(
        'units_1', min_value=32, max_value=128, step=32, default=64
    )
    layer_x = layers.Dense(units_1, activation='relu')(all_features)

    dropout_rate = hp.Float(
        'dropout_rate', min_value=0.1, max_value=0.4, step=0.1, default=0.2
    )
    layer_x = layers.Dropout(dropout_rate)(layer_x)

    units_2 = hp.Int(
        'units_2', min_value=16, max_value=64, step=16, default=32
    )
    layer_x = layers.Dense(units_2, activation='relu')(layer_x)
    output = layers.Dense(1, activation='linear')(layer_x)
